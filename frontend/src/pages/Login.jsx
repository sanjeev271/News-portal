  ...
  return (
    <div className="flex min-h-[60vh] items-center justify-center px-3 py-8 sm:px-4 sm:py-12">
      <div className="card-surface w-full max-w-md p-6 sm:p-10">
        <h2 className="mb-2 text-2xl font-extrabold text-slate-900 dark:text-white">{t("loginTitle")}</h2>
        <p className="mb-6 text-sm text-slate-500 dark:text-slate-400">{t("loginDesc")}</p>
        ...
        {loggedInUser ? (
          <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-5 text-center dark:border-emerald-900 dark:bg-emerald-950/30">
            <p className="mb-2 font-semibold text-emerald-800 dark:text-emerald-300">
              {t("loginSuccess", { name: loggedInUser.name })}
            </p>
            <div className="flex justify-center">
              <RoleBadge role={loggedInUser.role} />
            </div>
            <p className="mt-2 text-sm text-emerald-600 dark:text-emerald-400">
              {loggedInUser.role === "admin" ? t("loginAdminNote") : t("loginReaderNote")}
            </p>
          </div>
        ) : (
          <>
            ...
            <p className="mt-6 text-center text-sm text-slate-500">
              {t("noAccount")}{" "}
              <Link to="/register" className="font-bold text-bbc-red hover:underline">{t("register")}</Link>
            </p>
          </>
        )}
      </div>
